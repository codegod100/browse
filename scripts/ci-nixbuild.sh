...
resolve_creds() {
  ...
  if [[ -z "${NIXBUILD_TOKEN:-}" ]]; then
    ...
    echo "  Scope policy example: pipeline_slug browse (or boxci host secrets)" >&2
    echo "SKIP=1"
    return 0
  fi
...
setup_nixbuild() {
  ...
  sudo cp "$builders_file" /etc/nix/browse-nixbuild-builders
  sudo chmod 644 /etc/nix/browse-nixbuild-builders

  {
    ...
    echo 'max-jobs = 0'
    echo 'builders = @/etc/nix/browse-nixbuild-builders'
  ...
  } | sudo tee /etc/nix/browse-nixbuild.conf >/dev/null
  sudo chmod 644 /etc/nix/browse-nixbuild.conf

  if ! sudo grep -q 'browse-nixbuild.conf' /etc/nix/nix.conf 2>/dev/null; then
    echo 'include /etc/nix/browse-nixbuild.conf' | sudo tee -a /etc/nix/nix.conf >/dev/null
  fi
  ...
  sudo cp /etc/nix/browse-nixbuild.conf "$HOME/.config/nix/nix.conf"
  export NIX_CONFIG="include /etc/nix/browse-nixbuild.conf"
