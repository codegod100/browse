#!/usr/bin/env bash
# Trigger boxci merge build from browse's repo-local .boxci/pipeline.yml.
#
# Usage:
#   ./scripts/boxci/dispatch-merge.sh
#   ./scripts/boxci/dispatch-merge.sh --sha abc1234
#
# Env:
#   BOXCI_URL          — default https://boxci.boxd.sh
#   BROWSE_BRANCH      — default main
#   BROWSE_REPO_URL    — Garden clone URL (default below)
#   BROWSE_REPO_ID     — rad:… RID (default below)
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "$SCRIPT_DIR/../.." && pwd)"
cd "$REPO_ROOT"

usage() {
  sed -n '2,13p' "$0"
  exit 2
}

SHA=""
while [[ $# -gt 0 ]]; do
  case "$1" in
    --sha)
      [[ $# -ge 2 ]] || usage
      SHA="$2"
      shift 2
      ;;
    -h | --help)
      usage
      ;;
    *)
      if [[ -z "$SHA" && "$1" =~ ^[0-9a-fA-F]{7,40}$ ]]; then
        SHA="$1"
        shift
      else
        echo "dispatch-merge: unknown arg: $1" >&2
        usage
      fi
      ;;
  esac
done

BOXCI_URL="${BOXCI_URL:-https://boxci.boxd.sh}"
BRANCH="${BROWSE_BRANCH:-main}"
REPO_URL="${BROWSE_REPO_URL:-https://nandi.radicle.garden/z2QL7QdL2QGg6FmX3wcw3Mzm2ykE3.git}"
REPO_ID="${BROWSE_REPO_ID:-rad:z2QL7QdL2QGg6FmX3wcw3Mzm2ykE3}"

command -v curl >/dev/null 2>&1 || { echo "curl required" >&2; exit 1; }
command -v jq >/dev/null 2>&1 || { echo "jq required" >&2; exit 1; }

if [[ -z "$SHA" ]]; then
  command -v git >/dev/null 2>&1 || { echo "git required (or pass --sha)" >&2; exit 1; }
  SHA="$(git rev-parse "$BRANCH")"
fi

payload="$(jq -n \
  --arg repo_url "$REPO_URL" \
  --arg repo "$REPO_ID" \
  --arg trigger "merge" \
  --arg sha "$SHA" \
  --arg branch "$BRANCH" \
  '{
    repo_url: $repo_url,
    repo: $repo,
    trigger: $trigger,
    sha: $sha,
    branch: $branch
  }')"

url="${BOXCI_URL%/}/api/runs/from-repo"
echo "POST $url"
echo "  trigger=merge GIT_SHA=${SHA:0:7} branch=$BRANCH repo=$REPO_URL"

response="$(curl -fsSL -X POST "$url" \
  -H "Content-Type: application/json" \
  -d "$payload")"

run_id="$(echo "$response" | jq -r '.id // empty')"
status="$(echo "$response" | jq -r '.status // empty')"
pipeline="$(echo "$response" | jq -r '.pipeline // empty')"

if [[ -n "$run_id" ]]; then
  echo "run $run_id: $status"
  [[ -n "$pipeline" ]] && echo "  pipeline=$pipeline"
  echo "$response" | jq '{id, status, pipeline, duration_s, steps: [.steps[]? | {key, label, status}]}'
else
  echo "$response" | jq .
fi
